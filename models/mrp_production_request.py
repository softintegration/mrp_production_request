...
class MrpProductionRequest(models.Model):
    ...
    origin = fields.Char(
        'Source', copy=False,
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
        help="Reference of the document that generated this production order request.",readonly=False)
    ...
    def _onchange_product_id(self):
        ...
        if not self.product_id:
            self.bom_id = False
        elif not self.bom_id or self.bom_id.product_tmpl_id != self.product_id.product_tmpl_id or (self.bom_id.product_id and self.bom_id.product_id != self.product_id):
    ...
    def _check_date_range(self):
        for each in self:
            if each.date_request and each.date_desired and each.date_request > each.date_desired:
                raise ValidationError(_('%s : Desired Date (%s) should be greater than Date (%s)', each.name, format_datetime(self.env, each.date_desired), format_datetime(self.env, each.date_request)))
        return True
